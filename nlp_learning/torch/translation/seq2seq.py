...
class EncoderRNN(nn.Module):
    def __init__(self, input_dict_size, embed_size, hidden_size, dropout_p=0.1, use_cuda=False):
        super(EncoderRNN, self).__init__()
        self.hidden_size = hidden_size
        self.use_cuda = use_cuda

        self.embedding = nn.Embedding(input_dict_size, embed_size)
...
class DecoderRNN(nn.Module):
    def __init__(self, max_output_len, output_dict_size, embed_size, hidden_size, attn_size, dropout_p=0.1, use_cuda=False):
        super(DecoderRNN, self).__init__()
        self.max_output_len = max_output_len
        self.hidden_size = hidden_size
        self.use_cuda = use_cuda

        self.embedding = nn.Embedding(output_dict_size, embed_size)
        nn.init.xavier_uniform_(self.embedding.weight)
        self.dropout = nn.Dropout(dropout_p)
    ...
    def forward_step(self, inputs, hidden, encoder_output):
        batch_size = inputs.shape[0]
        embedded = self.embedding(inputs).view(1, batch_size, -1) #[1, batch_size, embed_size]
        embedded = self.dropout(embedded)
        ...
        attn_output = torch.mean(encoder_output*attn_weights, dim=0, keepdim=True) #[1, batch_size, hidden_size*2]
...
class Seq2Seq(nn.Module):
    def __init__(self, input_dict_size, output_dict_size, max_output_len, embed_size, hidden_size, attn_size, dropout_p=0.1, use_cuda=False):
        super(Seq2Seq, self).__init__()
        self.encoder = EncoderRNN(input_dict_size=input_dict_size, embed_size=embed_size, hidden_size=hidden_size, dropout_p=dropout_p, use_cuda=use_cuda)
        self.decoder = DecoderRNN(max_output_len=max_output_len, output_dict_size=output_dict_size, embed_size=embed_size, hidden_size=hidden_size, attn_size=attn_size, dropout_p=dropout_p, use_cuda=use_cuda)
